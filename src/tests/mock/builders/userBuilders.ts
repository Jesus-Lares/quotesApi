import { IUser, RoleUser } from "../../../context/user/domain/IUser";
...
export default class UserBuilder {
  ...
  generate(userArray: Array<any>, id: number): IUser {
    this.id(id);
    this.name(userArray[0]);
    this.email(userArray[1]);
    this.password(userArray[2]);
    this.registerDate(userArray[3]);
    this.role(userArray[4]);
    this.state(userArray[5]);
    this.dailyQuote(userArray[6]);
    this.allQuotes(userArray[7]);
    return this.user;
  }
}
